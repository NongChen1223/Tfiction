...
- 👁️ **摸鱼模式**：支持基础隐身 / 完全隐身、悬停唤出、右键快捷控制面板、透明背景阅读和文字可见度调节
...
- 🫥 **摸鱼效果已隔离**：摸鱼模式的透明背景和文字可见度现在只作用于阅读页，离开阅读页会自动恢复，不会再串到书架和设置页
- 🎼 **桌面歌词浮窗**：macOS 阅读摸鱼模式已改为原生桌面浮窗，进入后主窗口让位，只保留透明背景上的正文文字，效果更接近 QQ 音乐歌词
- ↔️ **浮窗拖拽缩放可见**：桌面浮窗默认贴近主阅读窗口打开，只有鼠标进入窗口内侧的一圈边框带时才显示轻量拖拽框、顶部拖动提示和右下角尺寸手柄，透明时也不容易丢失
...
14. macOS 阅读摸鱼模式会切到原生桌面浮窗，主窗口恢复普通模式，正文会以歌词式悬浮文本显示
15. 桌面浮窗会尽量沿用上一次位置；首次打开会贴近主阅读窗口中心，透明状态下也可拖拽和调整大小
16. 浮窗边框只会在鼠标进入窗口内侧边框带时出现，正文内圈不会显示；正文区域左键可直接拖动，滚轮也会优先停留在浮窗内
